...
(function (global, module, undefined) {
	...
	ProgressTracker = function (config, master) {
		...
		workers = cpus;
		while (workers--) {
			progress.receiver.workers.push(false);
		}
		...
		this.addListener("receiver-worker", function (worker) {
			...
			//Set worker state
			progress.receiver.workers[worker] = true;
		...
		var masterfn = function (master) {
			...
			if (master.isMaster) {
				...
				(function () {
					...
					//Create a vhost data socket reciver and set to utf8 encodeing
