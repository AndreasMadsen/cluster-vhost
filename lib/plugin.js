				...
				(function () {
					...
					configureProxy = function (protocol, allowInit) {
						...
						socket.on("error", function (err) {
							...
							//Throw a error if we aren't allow to spawn a proxy-server
							if (allowInit === false) {
								throw new Error("couldn't connect to proxy server");
							}
							...
							tracker.on("proxy-ready", function thisFunction(proxy) {
								...
								if (proxy === protocol) {
									...
									tracker.removeListener("proxy-master", thisFunction);
								}
							});
