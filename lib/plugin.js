...
(function (global, module, undefined) {
	...
	ProgressTracker = function (config, master) {
		...
		this.updateProtocolState = function (protocol) {
			...
			//if nothing is missing set state and emit proxy-done
			if (missing === false) {
				state.done = true;
		...
		this.updateClusterState = function () {
			...
			//Check that the cluster is listening
			...
			//If nothing is missing set state to ready and check ready state
			if (missing === false) {
				state.done = true;
		...
		this.checkReadyState = function () {
			...
			//Make sure that the cluster is listening
			if (progress.cluster.listening === false) {
				missing = true;
			}

			//Make sure all sockets are ready
			if (progress.socket.ready === false) {
				missing = true;
			}
		...
		//Check done state
		this.checkFinalizedState = function () {

			var missing = false;
			
			//Make sure that the cluster is ready
			if (progress.cluster.done === false) {
				missing = false;
			}
		...
		//When the cluster is listening
		//- emit ready if nothing is missing
		this.addListener("cluster-listening", function () {
			
			//Set cluster state
			progress.cluster.listening = true;
		...
		//When the cluster and all its workers are ready
		this.addListener("cluster-ready", function () {
			
			//Set worker state
			progress.cluster.done = true;
		...
		var masterfn = function (master) {
			...
			//If this is the master - send vhost infomation
				...
				(function () {
					...
					//Emits when cluster.listen is executed
					master.once("listening", function () {
						...
						//Inform tracker that the cluster is now listening
						tracker.emit("cluster-listening");
					});
					
					//Keep track of when cluster workers are connected
					(function () {
						//Add lisenter
						var lisenter = function (worker) {
							tracker.emit("cluster-spawn", worker.id);
						};
						master.on("worker connected", lisenter);

						//Remove the lisenter when all cluster workers are ready
						tracker.once("cluster-ready", function () {
							master.removeListener("worker connected", lisenter);
						});
					})();
					...
					configureProxy = function (protocol, allowInit) {
						...
						socket.on("connect", function () {
							...
							//If we was allowed to init a proxy-server set state to true
							if (allowInit === true) {
